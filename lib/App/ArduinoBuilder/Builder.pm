...
sub _ino_to_cpp {
  ...
  if (!$this->{config}->exists('recipe.preproc.macros')) {
    ...
    $recipe = $this->{config}->get('recipe.cpp.o.pattern', allow_partial => 1, with => {source_file => $source, object_file => "${target}.cpp-pre"});
    ...
    $recipe = $this->{config}->get('recipe.preproc.macros', with => {source_file => $source, preprocessed_file_path => "${target}.cpp-pre"});
  }
  _execute($recipe);
  ...
  open my $fi, '<', "${target}.cpp-pre" or fatal "Can’t open intermediate file '${target}.cpp-pre' for reading: $!";
  open my $fo, '>', "${target}.cpp" or fatal "Can’t open intermediate file '${target}.cpp' for writing: $!";
...
sub build_object_files {
  my ($this, $source_dir, $target_dir, $excluded_dirs, $force) = @_;
  make_path($target_dir);
